...
const ExperienceTimeline = () => {
  const { data: experience, loading } = useSupabaseData('experience');
  if (loading) return <div style={{ textAlign: "center", padding: 40 }}><span style={{ fontFamily: F.mono, fontSize: 11, color: C.t3 }}>Loading experience...</span></div>;
  if (!experience.length) return <div style={{ textAlign: "center", padding: 40 }}><span style={{ fontFamily: F.mono, fontSize: 11, color: C.t3 }}>No experience data. Add entries in Supabase.</span></div>;
  return (
    <div style={{ position: "relative", paddingLeft: 30 }}>
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 1, background: C.border }} />
      {experience.map((e, i) => (
        <div key={e.id || i} style={{ marginBottom: 30, position: "relative" }}>
          ...
          <div style={{ fontFamily: F.mono, fontSize: 10, color: C.t3, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 5 }}>{e.date_range}</div>
          <div style={{ fontFamily: F.serif, fontSize: 20, fontWeight: 400, marginBottom: 3 }}>{e.role}</div>
          <div style={{ color: C.gold, fontSize: 13, fontWeight: 500, marginBottom: 3 }}>{e.company}</div>
          <div style={{ fontFamily: F.mono, fontSize: 10, color: C.t3, marginBottom: 7 }}>{e.location}</div>
          <p style={{ color: C.t2, fontSize: 13, lineHeight: 1.8, maxWidth: 540 }}>{e.description}</p>
        </div>
      ))}
    </div>
  );
...
const ArticlesSection = () => {
  ...
  const { data: articles, loading } = useSupabaseData('articles');
  const normalized = articles.map(a => ({
    tag: a.tag,
    tagColor: a.tag_color || C.gold,
    date: a.date,
    read: a.read_time,
    img: a.img,
    title: a.title,
  ...
  return (
    <>
      ...
      {loading && <div style={{ textAlign: "center", padding: 40 }}><span style={{ fontFamily: F.mono, fontSize: 11, color: C.t3 }}>Loading articles...</span></div>}
      {!loading && !normalized.length && <div style={{ textAlign: "center", padding: 40 }}><span style={{ fontFamily: F.mono, fontSize: 11, color: C.t3 }}>No articles found. Add articles in Supabase.</span></div>}
      ...
      {openIdx !== null && <ArticleModal article={normalized[openIdx]} onClose={() => setOpenIdx(null)} />}
    </>
  );
};
